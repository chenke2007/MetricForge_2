    ...
    evidence: [
      {
        ...
        sourceFields: ['t.month', 't.total_revenue'],
        calculation: "SUM(t.amount) GROUP BY TO_CHAR(transaction_date, 'YYYY-MM') ORDER BY month",
        confidence: 'high',
        confidenceReason: '6 个月数据完整，每月收入均确认',
        relatedIntent: { metrics: ['收入'], dimensions: ['月份'], filters: [], timeRange: '近 6 个月' },
        displayValue: '¥16.4M (最新月)',
        sqlSnippet: 'SUM(t.amount) GROUP BY month',
      },
    ],
    evidenceSummary: '共 1 项证据：近 6 个月收入持续增长，6 月达 ¥16.4M 新高。',
