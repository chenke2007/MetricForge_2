    ...
    evidence: [
      {
        ...
        sourceFields: ['r.region', 't.amount'],
        calculation: 'ROUND((revenue_this - revenue_last) / NULLIF(revenue_last, 0) * 100, 1) AS yoy_pct',
        confidence: 'high',
        confidenceReason: '完整日历年对比数据，计算逻辑经校验',
        relatedIntent: { metrics: ['收入'], dimensions: ['区域', '年份'], filters: ['同比对比'], timeRange: '今年 vs 去年' },
        displayValue: '+22%',
      },
    ],
    evidenceSummary: '共 1 项证据：西南区域以 22% 同比增长领跑所有区域。',
    risks: ['增长包含通胀因素'],
