    ...
    evidence: [
      {
        ...
        calculation: 'SUM(r.amount) AS total_sales GROUP BY r.region ORDER BY total_sales DESC',
        confidence: 'medium',
        confidenceReason: '基于基础销售数据，未包含时间范围限制',
        relatedIntent: { metrics: ['销售额', '订单数'], dimensions: ['区域'], filters: [] },
        displayValue: '¥12.3M',
        sqlSnippet: 'SUM(r.amount) GROUP BY region ORDER BY total_sales DESC',
      },
    ],
    evidenceSummary: '共 1 项证据：华东区域以 ¥12.3M 销售额领先。',
    risks: ['数据口径需确认'],
